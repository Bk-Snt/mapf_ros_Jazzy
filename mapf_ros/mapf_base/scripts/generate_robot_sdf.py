#!/usr/bin/env python3
"""Generate per-robot SDF files with namespaced TF frame names."""
  ...
  <model name="{name}">
    ...
    <link name="base_link">
      ...
      <collision name="base_collision">
        <pose>-0.032 0 0.070 0 0 0</pose>
        <geometry>
          <box><size>0.140 0.140 0.140</size></box>
        </geometry>
      </collision>
    ...
    <link name="imu_link">
      <sensor name="tb3_imu" type="imu">
        ...
        <plugin name="turtlebot3_imu" filename="libgazebo_ros_imu_sensor.so">
          ...
          <frame_name>{name}/imu_link</frame_name>
        </plugin>
      </sensor>
    </link>
      ...
      <sensor name="hls_lfcd_lds" type="ray">
        ...
        <ray>
          <scan><horizontal>
            ...
            <min_angle>0.0</min_angle><max_angle>6.28</max_angle>
          </horizontal></scan>
        ...
        <plugin name="turtlebot3_laserscan" filename="libgazebo_ros_ray_sensor.so">
          ...
          <frame_name>{name}/base_scan</frame_name>
        </plugin>
      </sensor>
    </link>
    ...
    <link name="wheel_left_link">
      <inertial>
        ...
        <inertia>
          <ixx>1.8158194e-03</ixx><ixy>-9.3392e-12</ixy><ixz>1.04909e-11</ixz>
          <iyy>3.2922126e-03</iyy><iyz>5.75694e-11</iyz><izz>1.8158194e-03</izz>
        </inertia>
...
def main() -> None:
    ...
    num_robots = int(sys.argv[1]) if len(sys.argv) > 1 else 4

    for i in range(num_robots):
        name = f"robot_{i}"
        r, g, b = COLORS[i % len(COLORS)]
        sdf_content = TEMPLATE.format(name=name, r=r, g=g, b=b)
        filepath = os.path.join(output_dir, f"{name}.sdf")
