...
set(_exported_dependencies "rclcpp;rclcpp_action;rclcpp_lifecycle;std_msgs;nav_msgs;sensor_msgs;geometry_msgs;visualization_msgs;tf2_geometry_msgs;nav2_util;tf2_ros;nav2_costmap_2d;builtin_interfaces;pluginlib;mapf_msgs;mapf_ros")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to mapf_base_DEFINITIONS, mapf_base_INCLUDE_DIRS,
# mapf_base_LIBRARIES, and mapf_base_LINK_FLAGS.
# Additionally collect the direct dependency names in
# mapf_base_DEPENDENCIES as well as the recursive dependency names
# in mapf_base_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(mapf_base_DEPENDENCIES ${_exported_dependencies})
  set(mapf_base_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(mapf_base_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(mapf_base_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(mapf_base_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(mapf_base_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(mapf_base_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND mapf_base_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
